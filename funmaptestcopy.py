...
#COPIED FROM stretch_ros/stretch_funmap/nodes/funmap
...
class ContactDetector():
    ...
    def update(self, joint_states, stop_the_robot_service):
        ...
        self.position = self.get_lift_pos(joint_states)
        ...
        if not self.passed_stopping_position():
            ...
            if self.is_in_contact():
                ...
                if contact_position is not None: 
                    new_target = contact_position + 0.070 #- 0.002
                else:
                    new_target = self.position() - 0.070 #- 0.002
                pose = {joint_name : new_target}
